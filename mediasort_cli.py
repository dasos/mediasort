...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Move photos or videos based on the EXIF data."
    )
    parser.add_argument("input_dir")
    parser.add_argument("output_dir")
    parser.add_argument("-d", "--dryrun", action="store_true", default=False)
    parser.add_argument(
        "-v", "--version", action="version", version="photosort v" + VERSION
    )
...
def move_all_in_set(
    set, output_dir, use_date_directory=True, use_name_directory=True, dryrun=False
):
    if use_date_directory and use_name_directory:
        folder_name = os.path.join(
            output_dir, "{} {}".format(set.date_directory(), set.name)
        )
