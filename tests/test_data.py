...
def test_load_files_redis(app):
    with app.app_context():
        with app.test_request_context():
            ...
            assert (
                len(list(redis_client.zrange("mediasort:sets", 0, -1)))
                == LENGTH_OF_SETS
            )

            assert (
                redis_client.zcount("mediasort:sets", "-inf", "+inf") == LENGTH_OF_SETS
            )

            # Get all the items
            assert (
                len(list(redis_client.scan_iter(match=f"mediasort:item-meta-*")))
                == LENGTH_OF_MEDIAITEMS
            )
...
def test_get_sets(redis_client):
    ...
    for set_id in redis_client.zrange(f"mediasort:sets", 0, -1):
        ...
        assert set.length == int(
            redis_client.hget(f"mediasort:set-meta-{set_id}", "length")
        )
...
def test_get_empty(redis_client):
    ...
    for set_id in redis_client.zrange(f"mediasort:sets", 0, -1):
        ...
        assert set.start.timestamp() == int(
            redis_client.hget(f"mediasort:set-meta-{set_id}", "start")
        )
