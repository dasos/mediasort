  ...
  for name in redis_client.scan_iter(match='mediasort:set-meta-*'):
    ...
    sets.append(
      {'name': redis_client.hgetall(name),
       'items': [name for name in redis_client.zrange(f'mediasort:set-items-{set_id}', 0, -1)],})
...
def move_set(action, set_id):
  ...
  l = logging.getLogger("mediasort.api.move_set")
  redis_client = system.get_db()
  ...
  def actually_move(set, name, testing=False):
    ...
    def remove_from_db():
      ...
      l.info ("Removing set information from Redis")
      l.debug (f'Set id: {set.id}')
    ...
    if "save" in action:
      ...
      l.info(f'Moved set: {set}')
      
    # Delete
    elif action == "delete":
      dir = current_app.config.get('DELETE_DIR')
      remove_from_db()
      set.move(dir, use_date_directory=False, use_name_directory=False, dry_run=testing)
      
      l.info(f'Moved set to delete directory')
      
    else:
      l.error(f'No command')

  set = data.get_set(set_id, store = True)
  if set is None:
    l.warning(f'Could not find set id: {set_id}')
    return jsonify(data = {'error': "Could not find set"})

  l.debug(f'Moving set: {set}')
  ...
  if current_app.testing:
    ...
    l.info ("Starting new thread for load")
